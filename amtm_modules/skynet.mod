#!/bin/sh
#bof
skynet_installed(){
	zero=
	if grep -qm1 '/underd0se/' "$scriptloc"; then
		localVother=$(grep -m1 -oE 'v[0-9]{1,2}([.][0-9]{1,2})([.][0-9]{1,2})([-a-zA-Z0-9.]+)?' "$scriptloc" | cut -d "." -f 4-)
		[ "$(echo $localVother | grep 'dev')" ] && remoteBranch=development || remoteBranch=master
		if [ "$su" = 1 ]; then
			remoteurl=https://raw.githubusercontent.com/underd0se/Skynet-Zero/$remoteBranch/firewall.sh
			remoteVother="$(c_url "$remoteurl" | grep -m1 -oE 'v[0-9]{1,2}([.][0-9]{1,2})([.][0-9]{1,2})([-a-zA-Z0-9.]+)?' | cut -d "." -f 4-)"
			grepcheck=underd0se
		fi
		zero=Zero
	else
		localVother=$(grep -m1 -oE '[0-9]{1,2}([.][0-9]{1,2})([.][0-9]{1,2})' "$scriptloc")
		if [ "$su" = 1 ]; then
			remoteurl=https://raw.githubusercontent.com/Adamm00/IPSet_ASUS/master/firewall.sh
			remoteVother="$(c_url "$remoteurl" | grep -m1 -oE '[0-9]{1,2}([.][0-9]{1,2})([.][0-9]{1,2})')"
			grepcheck=Adamm
		fi
	fi

	script_check
	if [ -z "$su" -a -z "$tpu" ] && [ "$skynetUpdate" ]; then
		localver="$lvtpu"
		upd="${E_BG}$skynetUpdate${NC}"
		if [ "$skynetMD5" != "$(md5sum "$scriptloc" | awk '{print $1}')" ]; then
			[ -f "${add}"/availUpd.txt ] && sed -i '/^skynet.*/d' "${add}"/availUpd.txt
			upd="${E_BG}${NC}$lvtpu"
			unset localver skynetUpdate skynetMD5
		fi
	fi
	[ -z "$updcheck" -a -z "$ss" ] && printf "${GN_BG} 2 ${NC} %-9s%-21s%${COR}s\\n" "open" "Skynet $zero   $localver" " $upd"
	case_2(){
		/jffs/scripts/firewall
		sleep 2
		show_amtm menu
	}
}
install_skynet(){
	check_ipset(){
		if ! ipset -v 2>/dev/null | grep -qE 'v6|v7'; then
			am=;show_amtm " Skynet install not possible,\\n IPSet version on router is not supported:\\n$(ipset -v 2>/dev/null | sed -e 's/^/ /')"
		fi
	}
	p_e_l
	printf " This installs Skynet - Router Firewall & Security\\n Enhancements on your router.\\n\\n"
	printf " There are two versions available to install:\\n\\n 1. Regular Skynet version\\n"
	printf "    Author: Adamm\\n    snbforums.com/forums/asuswrt-merlin-addons.60/?prefix_id=14\\n"
	printf "    Regular contributors: SomeWhereOverTheRainBow, dave14305\\n\\n"
	printf " 2. Skynet Zero, a swapless fork of Skynet\\n"
	printf "    Author: underdose\\n    snbforums.com/threads/skynet-zero-a-swapless-fork-of-skynet.97604\\n\\n"
	while true; do
		printf " Enter selection [1-2 e=Exit] ";read -r continue
		case "$continue" in
			1)		check_ipset
					c_url https://raw.githubusercontent.com/Adamm00/IPSet_ASUS/master/firewall.sh -o /jffs/scripts/firewall && chmod +x /jffs/scripts/firewall && sh /jffs/scripts/firewall install
					sleep 2
					if [ -f /jffs/scripts/firewall ]; then
						show_amtm " Skynet installed"
					else
						am=;show_amtm " Skynet installation failed"
					fi
					break;;
			2)		check_ipset
					c_url https://raw.githubusercontent.com/underd0se/Skynet-Zero/master/firewall.sh -o /jffs/scripts/firewall && chmod +x /jffs/scripts/firewall && sh /jffs/scripts/firewall install
					sleep 2
					if [ -f /jffs/scripts/firewall ]; then
						show_amtm " Skynet Zero installed"
					else
						am=;show_amtm " Skynet Zero installation failed"
					fi
					break;;
			[Ee])	am=;show_amtm menu;break;;
			*)		printf "\\n input is not an option\\n\\n";;
		esac
	done
}
#eof
